...
void sbits_initial_configuration_is_correct() {
    ...
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(UINT64_MAX, state->minKey, "SBITS minKey was not initialized correctly.");
...
void sbits_put_inserts_single_record_correctly() {
    ...
    *((int32_t *)(data + 4)) = 27335;
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(1, SBITS_GET_COUNT(state, state->buffer), "sbitsPut did not increment count in buffer correctly.");
...
void sbits_put_inserts_eleven_records_correctly() {
    ...
    *((int32_t *)(data + 4)) = 28361;
    ...
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(16321, state->minKey, "sbitsPut did not update minimim key on first insert.");
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(0, state->nextDataPageId, "sbitsPut incremented next page to write and it should not have.");
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(11, SBITS_GET_COUNT(state, state->buffer), "sbitsPut did not increment count in buffer correctly.");
    free(sbitsPutResultKey);
    free(sbitsPutResultData);
    free(data);
}
    ...
    for (int i = 0; i < 63; i++) {
        ...
        *((int32_t *)(data + 4)) %= (i + 1);
        ...
        if (i < 62) {
            ...
            TEST_ASSERT_EQUAL_INT32_MESSAGE(*((int32_t *)data), *sbitsPutResultKey, "sbitsPut did not put correct key value in buffer.");
            TEST_ASSERT_EQUAL_INT32_MESSAGE(*((int32_t *)(data + 4)), *sbitsPutResultData, "sbitsPut did not put correct data value in buffer.");
            TEST_ASSERT_EQUAL_UINT32_MESSAGE(i + 1, SBITS_GET_COUNT(state, state->buffer), "sbitsPut did not increment count in buffer correctly.");
        } else {
            memcpy(sbitsPutResultKey, (int8_t *)state->buffer + 6, 4);
            memcpy(sbitsPutResultData, (int8_t *)state->buffer + 10, 4);
            TEST_ASSERT_EQUAL_INT32_MESSAGE(0, *sbitsPutResultKey, "sbitsPut did not clear buffer after filling up.");
            TEST_ASSERT_EQUAL_INT32_MESSAGE(0, *sbitsPutResultData, "sbitsPut did not clear buffer after filling up.");
        }
    }
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(100, state->minKey, "sbitsPut did not update minimim key on first insert.");
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(1, state->nextDataPageId, "sbitsPut didn't increment next page to write.");
    free(sbitsPutResultKey);
    free(sbitsPutResultData);
    free(data);
}
...
void sbits_put_inserts_one_more_than_one_page_of_records_correctly() {
    ...
    *((int32_t *)(data + 4)) = 96875;
    ...
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(4444444, state->minKey, "sbitsPut did not update minimim key on first insert.");
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(1, SBITS_GET_COUNT(state, state->buffer), "sbitsPut did not reset buffer count to correct value after writing the page");
    free(sbitsPutResultKey);
    free(sbitsPutResultData);
    free(data);
}
