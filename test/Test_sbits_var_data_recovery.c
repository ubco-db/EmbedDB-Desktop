...
void sbits_variable_data_reloads_and_queries_with_thirty_one_pages_of_data_correctly() {
    ...
    for (int i = 0; i < 650; i++) {
        ...
        snprintf(message, 100, "SBITS get var returned null stream for key %i.", key);
        ...
        snprintf(message, 100, "SBITS get var did not return the correct variable data for key %i.", key);
...
void sbits_variable_data_reloads_and_queries_with_two_hundred_forty_seven_pages_of_data_correctly() {
    ...
    for (int i = 0; i < 2708; i++) {
        int8_t getResult = sbitsGetVar(state, &key, recordBuffer, &stream);
        if (i > 1163) {
            snprintf(message, 120, "SBITS get encountered an error fetching the data for key %i.", key);
            TEST_ASSERT_EQUAL_INT8_MESSAGE(0, getResult, message);
            snprintf(message, 120, "SBITS get did not return correct data for a record inserted before reloading (key %i).", key);
            TEST_ASSERT_EQUAL_INT32_MESSAGE(data, *((int32_t *)recordBuffer), message);
            snprintf(message, 120, "SBITS get var returned null stream for key %i.", key);
            TEST_ASSERT_NOT_NULL_MESSAGE(stream, message);
            uint32_t streamBytesRead = sbitsVarDataStreamRead(state, stream, variableDataBuffer, 13);
            TEST_ASSERT_EQUAL_UINT32_MESSAGE(13, streamBytesRead, "SBITS var data stream did not read the correct number of bytes.");
            snprintf(message, 120, "SBITS get var did not return the correct variable data for key %i.", key);
            TEST_ASSERT_EQUAL_CHAR_ARRAY_MESSAGE(variableData, variableDataBuffer, 13, message);
            free(stream);
        } else {
            snprintf(message, 120, "SBITS get encountered an error fetching the data for key %i. The var data was not detected as being overwritten.", key);
            TEST_ASSERT_EQUAL_INT8_MESSAGE(1, getResult, message);
            snprintf(message, 120, "SBITS get did not return correct data for a record inserted before reloading (key %i).", key);
            TEST_ASSERT_EQUAL_INT32_MESSAGE(data, *((int32_t *)recordBuffer), message);
            snprintf(message, 120, "SBITS get var did not return null stream for key %i when it should have no variable data.", key);
